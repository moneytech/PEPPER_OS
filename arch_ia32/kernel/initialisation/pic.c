...
void PIC_remap(unsigned int offset1, unsigned int offset2) {
  ...
  /*
     ...
     Word), qui permettent de paramétrer certaines opérations du contrôleur une
     fois que celui-ci a été réinitialisé.
      */
...
unsigned short int(pic_get_isr)() { return __pic_get_irq_reg(PIC_READ_ISR); }

unsigned short int(__pic_get_irr)() { return __pic_get_irq_reg(PIC_READ_IRR); }
