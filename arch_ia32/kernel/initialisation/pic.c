...
{
	//IRQ for Slaves PIC
	if (irq > 0x8) outb(PIC2_COMMAND , PIC_EOI) ;
...
void PIC_remap(unsigned int offset1 , unsigned int offset2)
{
	...
	/*
		...
		*	les registres OCW (Operation Control Word), qui permettent de paramétrer certaines opérations du
			contrôleur une fois que celui-ci a été réinitialisé.
	*/
